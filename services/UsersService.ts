...
async function getUsersByQuery(queryString: string): Promise<User[]> {
  if (queryString === '') {
    return [];
  }
...
async function createFollowerRelationship(userId: string, userToFollowId: string): Promise<void> {
...
async function removeFollowerRelationship(userId: string, userToFollowId: string): Promise<void> {
  const userDocRef = doc(db, USERS_COLLECTION, userId);
  checkUserExists(userDocRef);
